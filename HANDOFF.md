...
_Last updated: 2026-09-16, session 010 (docs/TODO.md finished — 78 steps — and **merged into `main`** on the founder's word; the build can start at step 0.1 / 1.1)._
...
- [x] `docs/TODO.md` — step-by-step build list with one prompt per step (**78 steps**, M0–M6, ~338k words), generated by
  `Session log/003_2026-09-04/todo-drafts/assemble.py` from `groups/G1–G14.json`. Drafted sessions 002–009; global review round 1
  (4 lenses, 137 findings, 18 blockers) and fixers (162 findings applied, 0 validator errors) in session 010 (2026-09-15). **Merged into `main` on 2026-09-16** (merge commit `a806f97`, founder: "merge it"); build sessions work on `main` as the prompts say. Optional before
  building: a second global review round (`workflow-global.js` `{"stage":"review","lenses":[…],"round":2}`) — round 1 left no known
  blockers, so it is a quality pass, not a requirement. Kit details and per-bite records: session 010 `workflow-results.md`.
...
- Merging: the TODO.md branch merged to `main` on 2026-09-16 on the founder's word ("merge it"); `claude/project-status-todo-plan-leghlm` is now fully merged and may be deleted together with the two stale `claude/plan-todo-prompts-*` branches.
...
confirmed as below; merge to `main` done 2026-09-16; deletion of the stale branches `claude/plan-todo-prompts-ia7gkd`, `claude/plan-todo-prompts-w3madd` and (now merged)
`claude/project-status-todo-plan-leghlm` is approved — if the sandbox proxy still blocks `git push --delete`, delete them in GitHub → Branches
(all safe: merged or record-only with history preserved in `Session log/002_2026-09-03/`). Do not re-ask these.
